...
import custom_changes   # @JV: New addition
...
class PerturbationL0Norm(Perturbation):
    ...
    def concretize(self, x, A, sign=-1, aux=None):  
        """ @JV
            A is the coefficient matrix, x is the origin for perturbation
            sign -1 returns lower bound and sign +1 returns upper bound for Ax' where ||x' - x||_p < self.eps
        """
        if A is None:
            return None
...
class PerturbationLpNorm(Perturbation):
    ...
    def concretize_matrix(self, x, A, sign):
        ...
        if self.norm == np.inf:
            ...
            if not isinstance(A, eyeC):
                # @JV: New addition
                bound = None
                if len(x.shape) == 3:   # @JV: i.e. (batch dim, flat bound node dim, flat input dim), x.shape is (batch dim, xdim, ydim). Can treat flat_bound_node_dim as channel dim
                    try:
                ...
                assert False, "Identity matrices not implemented"    # @JV: New addition
                bound = center + sign * diff
        else:   # norm is not inf
            ################ @JV: New addition ########
            xshape = x.shape
            channels, x_in_rows, x_in_cols = None, None, None
            if len(x.shape) == 3:   # @JV: non-image inputs (b, h, w)
                x_in_rows = x.shape[1]
                x_in_cols = x.shape[2]
            elif len(x.shape) == 4: # @JV: image inputs (b, c, h, w)
                channels = x.shape[1]
                x_in_rows = x.shape[2]
                x_in_cols = x.shape[3]
            ############################
            ...
            if not isinstance(A, eyeC):
                ...
                ## @JV: End of new addition
                    ...
                    assert False, f"x can't have shape {xshape}"
                ...
                assert False    # @JV: New addition
                bound = x + sign * self.eps
        bound = bound.squeeze(-1)
        return bound

    def concretize_patches(self, x, A, sign):
        if len(x.shape) != 4:   # @JV: we expect the input to be an image if there is a convolution layer somewhere in between
            assert False, f"x can't have shape {x.shape}"
        if self.eps != 0:
            self.x_L = x - self.eps
            self.x_U = x + self.eps
        if self.norm == np.inf:
            ...
            if not A.identity == 1:
                ...
                matrix = patches_to_matrix(
                    ...
                    A.unstable_idx)
                assert len(matrix.shape) == 5, "Matrix must be (b, s, c, h, w)"
                ...
                bound = center + sign * diff
            return bound
        else:  # Lp norm, and we know that x.shape is (b, c, h, w)
            input_shape = x.shape
            if not A.identity:
                # Find the upper and lower bounds via dual norm.
                # matrix has shape
    ...
    def concretize(self, x, A, sign=-1, aux=None):
        ...
        # print("A: ",A)
        if self.eps is None:
            assert False, "Please provide epsilon instead of lower and upper bounds"
        if A is None:
            return None
